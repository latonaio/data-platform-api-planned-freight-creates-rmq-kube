...
import (
	...
	dpfm_api_processing_formatter "data-platform-api-planned-freight-creates-rmq-kube/DPFM_API_Processing_Formatter"
...
func (c *DPFMAPICaller) headerCreateSql(
	...
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": headerData, "function": "PlannedFreightHeader", "runtime_session_id": sessionID})
	if err != nil {
...
func headerIsUpdate(header *dpfm_api_processing_formatter.HeaderUpdates) bool {
	plannedFreight := header.PlannedFreight

	return !(plannedFreight == 0)
}
